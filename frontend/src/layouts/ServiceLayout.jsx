...
const ServiceLayout = () => {
  const [title, setTitle] = useState("choose service on the side");

  // it way of get the url and other things related to url so easy in react
  const Location = useLocation();

  // the links that navigate to the different services
  const links = [
    { name: "Transportation service", to: "/services" },
    { name: "Home care services", to: "home-care" },
    { name: "Language translation service", to: "language-translation" },
  ];
  ...
  useEffect(() => {
    if (Location.pathname.includes("services")) {
      if (Location.pathname == "/services") {
        setTitle("choose service on the side");
        return;
      }
      links.some((link) => {
