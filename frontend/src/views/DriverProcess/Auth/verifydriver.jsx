...
const VerifyDriver = () => {
  ...
  const { verifyOtp, isLoading } = useDriverAuth();
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      // Proceed with navigation if OTP is verified
      navigate("/driver/dashboard", { replace: true });

      // User canceled the navigation
      Swal.fire("Cancelled", "Your navigation is cancelled", "info");
    } catch (err) {
      console.log(err);
      // Handle error - maybe show an error message
      Swal.fire("Error", "There was an issue verifying the OTP", "error");
    }
  };
